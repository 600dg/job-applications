...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ error: "We could not read this PDF. Try exporting it again from your document editor." }, { status: 422 });
  }
...
export async function PATCH(request: Request) {
  ...
  const db = getDb();
  const [owned] = await db.select({ id: resumes.id }).from(resumes)
    .where(and(eq(resumes.id, body.id), eq(resumes.ownerId, ownerId))).limit(1);
  if (!owned) return Response.json({ error: "Résumé not found." }, { status: 404 });

  await db.update(resumes).set({ isPrimary: false, updatedAt: new Date() })
    .where(and(eq(resumes.ownerId, ownerId), ne(resumes.id, body.id)));
  const [resume] = await db.update(resumes).set({ isPrimary: true, updatedAt: new Date() })
    .where(and(eq(resumes.id, body.id), eq(resumes.ownerId, ownerId))).returning();
...
export async function DELETE(request: Request) {
  ...
  const db = getDb();
  const [owned] = await db.select().from(resumes)
    .where(and(eq(resumes.id, id), eq(resumes.ownerId, ownerId))).limit(1);
  if (!owned) return Response.json({ error: "Résumé not found." }, { status: 404 });
  ...
  if (owned.isPrimary) {
    const [replacement] = await db.select({ id: resumes.id }).from(resumes)
      .where(eq(resumes.ownerId, ownerId)).orderBy(desc(resumes.createdAt)).limit(1);
    if (replacement) {
      await db.update(resumes).set({ isPrimary: true, updatedAt: new Date() })
