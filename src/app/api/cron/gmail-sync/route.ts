...
export async function GET(request: Request) {
  ...
  const connections = await getDb().select({ ownerId: gmailConnections.ownerId }).from(gmailConnections).where(eq(gmailConnections.status, "active"));
  const results = [];
  for (const connection of connections) {
    try {
      const result = await syncGmailOwner(connection.ownerId);
      results.push({ ownerId: connection.ownerId, ok: true, updated: result.updated });
    } catch {
